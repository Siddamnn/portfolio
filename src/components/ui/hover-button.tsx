...
const HoverButton = React.forwardRef<HTMLButtonElement, HoverButtonProps>(
  ({ className, children, ...props }, ref) => {
    ...
    const createCircle = React.useCallback((x: number, y: number) => {
      ...
      // Using CSS variables for colors that can be overridden
      const color = `linear-gradient(to right, var(--circle-start, #a0d9f8) ${xPos * 100}%, var(--circle-end, #3a5bbf) ${
        xPos * 100
      }%)`
    ...
    return (
      <button
        ...
        className={cn(
          ...
          "dark:bg-[rgba(255,255,255,0.05)] dark:before:shadow-[inset_0_0_0_1px_rgba(255,255,255,0.1),inset_0_0_16px_0_rgba(255,255,255,0.05)]",
          className
        )}
        ...
        {circles.map(({ id, x, y, color, fadeState }) => (
          <div
            ...
            className={cn(
              ...
              fadeState === "out" && "opacity-0 [transition-duration:1.2s]",
              !fadeState && "opacity-0"
            )}
            style={{
              left: x,
